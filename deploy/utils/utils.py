...
def merge_two_config_dicts(root_config, config_patch):
  """Merge config_patch into root_config.

  Args:
    root_config: Dictionary for all configs.
    config_patch: Dictionary to be merged.
  ...
  for key, val in config_patch.items():
    if key in root_config:
      ...
      if isinstance(val, list) and isinstance(root_value, list):
        ...
        merge_two_config_dicts(root_value, val)
      else:
...
def load_config(overall_path):
  ...
  for inc_file in import_files:
    ...
    merge_two_config_dicts(overall, inc_contents)
